...
EMP/MKT/ADCOOP), plus the default coupon template. Idempotent - safe to
re-run, matching every other seed command in this project."""
from django.core.management.base import BaseCommand

from voucher_portal.geometry import DEFAULT_FIELD_GEOMETRY
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        template, created = VoucherTemplate.objects.get_or_create(
            name="ADCOOP default coupon",
            defaults={"field_geometry": DEFAULT_FIELD_GEOMETRY, "is_default": True, "is_active": True},
        )
        if not created and not template.field_geometry:
            template.field_geometry = DEFAULT_FIELD_GEOMETRY
        ...
        self.stdout.write(self.style.SUCCESS(
            ...
            f"Prefixes: {VoucherPrefix.objects.count()}. Default template: {'created' if created else 'exists'}."))
